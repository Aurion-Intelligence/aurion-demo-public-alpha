...
|---|---|---|
| Offline demo runner | present | `python scripts/demo/run_public_spine_demo.py` |
| Generate / replay a bounded Mission Receipt | works | runner `validate` (replay) and `--mode generate` |
| Exported AuditLedger evidence | present (fixture-backed) | `artifacts/demo/PUBLIC_SPINE_DEMO_LOOP_001.audit.jsonl` |
| Exported BlackBox decision traces | present (4 traces) | `artifacts/demo/PUBLIC_SPINE_DEMO_LOOP_001.blackbox/` |
...
The AuditLedger and BlackBox evidence are **replayed / fixture-backed**: they are the real evidence
captured during the original bounded run, shipped as fixtures. Nothing is fabricated.

## What you CAN do with this release

- Run the offline demo runner and inspect a governed Mission Receipt (validate/replay or generate a
  fresh bounded one).
